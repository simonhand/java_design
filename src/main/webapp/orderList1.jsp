...
<div class="container" style="background-color: white;">
    <div class="row" style="margin-left: 40px">
        <h3>我的订单列表&nbsp;&nbsp;
            <small>温馨提示：<em>${loginUser.username}</em>有<font color="red">${pageBean.totalSize}</font>个订单</small></h3>
    </div>
        ...
        <c:if test="${pageBean.totalSize>0 }">
            <nav aria-label="..." class="text-center">
                <ul class="pagination">
                    ...
                    <c:if test="${pageBean.pageNum>1 }">
                        <li><a href="${pageContext.request.contextPath }/userservlet?method=getOrderList&pageNum=${pageBean.pageNum-1}&pageSize=${pageBean.pageSize}&typeId=${typeId}" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
                    </c:if>
                    ...
                    <c:forEach var="pn" begin="${pageBean.startPage }" end="${pageBean.endPage }" step="1">
                        ...
                        <c:if test="${pn!=pageBean.pageNum }">
                            <li ><a href="${pageContext.request.contextPath }/userservlet?method=getOrderList&pageNum=${pn }&pageSize=${pageBean.pageSize}&typeId=${typeId}">${pn }</a></li>
                        </c:if>
                    </c:forEach>
                    ...
                    <c:if test="${pageBean.pageNum<pageBean.pageCount }">
                        <li><a href="${pageContext.request.contextPath }/userservlet?method=getOrderList&pageNum=${pageBean.pageNum+1}&pageSize=${pageBean.pageSize}&typeId=${typeId}" aria-label="Next"><span aria-hidden="true">»</span></a></li>
                    </c:if>
